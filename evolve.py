...
from const import ktype, neta, beta, acc2, xi, tflag, eddfac, mch, epsnov, gamma, yeardy, aursun
...
def evolve(kstar, mass0, mass, rad, lumin, massc, radc, menv, renv,
    ...
    for k in range(kmin, kmax + 1):
        ...
        if tphys < tiny and ospin[k] <= 0.001:
            ospin[k] = 45.350 * vrotf(mass[k]) / rm
            ...
            while True:
                ...
                if intpol == 0 and abs(dtm) > tiny and not sgl:
                    ...
                    if neta > tiny and not esymb:
                        ...
                        lacc = lacc / lumin[j1]
                        if (lacc > 0.010 and not bsymb) or (lacc < 0.010 and bsymb):
                            jp = min(80, jp + 1)
                    ...
                    for k in range(kmin, kmax + 1):
                        ...
                        if (mass[k] > 0.35 and kstar[k] < 10):
                            ...
                            djspint[k] = djspint[k] + djmb
                            if djmb > tiny:
                                dtj = 0.030 * jspin[k] / abs(djmb)    #################   0.03
                                dt = min(dt, dtj)
                    dtm = dt / 1.0e6
                ...
                for k in range(kmin, kmax + 1):
                    ...
                    if kstar[k] < 10:
                        ...
                        if dml < dms[k]:
                            ...
                            dt = 1.0e6 * dtm
                        # 限制 1% 的质量损失
                        if dms[k] > 0.010 * mass[k]:
                            dtm = 0.010 * mass[k] * dtm / dms[k]
                            if k == 2:
                                dms[1] = dms[1] * 0.010 * mass[2] / dms[2]
                            dms[k] = 0.010 * mass[k]
                            dt = 1.0e6 * dtm
                ...
                while True:
                    ...
                    if kstar[j1] == 0 and q[j1] > 0.695:
                        ...
                        break
                            ...
                            kstar[j1] == 2 and q[j1] > qc) or (kstar[j1] == 4 and q[j1] > qc):
                        # 公共包层演化
                        m1ce = mass[j1]
                        m2ce = mass[j2]
                    ...
                    # donor星为黑洞
                        ...
                        for k in range(1, 3):
                            if neta > tiny:
                                ...
                                vwind2 = 2.0 * beta * acc1 * mass[k] / radx[k]
                                omv2 = (1.0 + vorb2 / vwind2) ** (3.0 / 2.0)
                                dmt[3 - k] = ivsqm * acc2 * dmr[k] * ((acc1 * mass[3 - k] / vwind2) ** 2) / (
                                        2.0 * sep * sep * omv2)
